...
## P1.5 — 실배포 전 하드닝 (실제 정산 장부 시뮬레이션 기반)

> 사진 장부(KakaoTalk_20260718_153253473*.jpg) 기반 시뮬레이션으로 재고/가격 표시 버그 다수 발견·수정 후, 선입금 주문 매출이 정산에 전혀 안 잡히는 핵심 갭을 발견해 계획 수립. 상세 내역은 `docs/exec-plans/completed/phase-5-order-revenue-and-safety.md` 참조.

- [x] 재고 수량 표시 소수점 통일 + "EA" 단위로 고정 (규격 문자열이 재고 개수 뒤에 붙어 혼란스럽던 문제 해소)
- [x] 정산 리포트·상세 리포트·전체 채널 정산의 금액을 정수로 표시, `record_sale()` 계산 단계부터 반올림
- [x] 상세 리포트에 크기(특/대/중) 정보 채움 + 맨 아래 합계(수량/금액) 행 추가 (화면 + 엑셀 둘 다)
- [x] 선입금 주문 출고 완료 시 매출(sales)로 자동 반영 — 기존엔 재고만 차감하고 매출 집계에서 전혀 안 잡히던 핵심 버그. 채널 없음(channel_id=NULL), 단가는 변형 기준단가 사용
- [x] 택배비(`shipping_fee`) 필드 추가 (마이그레이션 004, orders 테이블) — 매출/순이익 집계에는 미포함, 단순 통과 기록
- [x] 삭제 버튼 5곳(변형/거래/판매/품목/채널)에 체크박스 확인 후 활성화되는 안전장치 추가
- [x] 정산 확정 이력 조회 UI 추가 (`list_settlements` — 구현만 되어있고 화면에 없던 것 노출)
- [x] 실제 장부(사진2·3) 기반 시뮬레이션 데이터로 브라우저 재검증 (88건 판매, 29건 선입금 주문) — 대표 건은 실제 UI 폼으로, 나머지는 동일 백엔드 함수로 입력
- [x] pytest 105개 전체 통과 (신규 7건)

---

## P1.6 — 농장 PC 실배포 대응 + 위탁판매 2단계 흐름 전환

> 농장 PC(부모님 사용)에서 실제 배포 진행 중 발견된 이슈들과, "위탁판매는 출고 ≠ 판매"라는 핵심 비즈니스 로직 반영.

- [x] 배치 파일(`앱_실행.bat`/`앱_업데이트.bat`) 인코딩 수정 — UTF-8(BOM 없음)+LF가 일부 Windows PC의 cmd.exe에서 한글 깨짐·명령 오분리를 일으킴. BOM 추가로도 해결 안 돼 CP949(한글 Windows 콘솔 기본 인코딩)+CRLF로 재작성, `chcp 65001` 제거. `.gitattributes`에 `*.bat -text` 추가해 git 자동 변환으로부터 보호
- [x] 판매 등록 시 단가 0원 허용 안 함 — 변형에 기준단가 미설정 시 자동채움이 안 먹어 단가 0인 채 조용히 등록되던 문제. `record_sale()`이 `unit_price > 0` 강제하도록 검증 강화
- [x] **위탁판매 2단계 흐름 도입**: "판매는 위탁판매라 출고 ≠ 판매"라는 피드백 반영 — 채널이 위탁(consignment) 유형이면 "판매" 탭이 출고(재고만 차감, `consignment_shipments` 테이블에 '판매대기' 기록) → 위탁 판매대기 목록에서 실제 판매 수량·단가 확정(그제서야 매출 반영) 2단계로 분기. 직접판매/채널 미지정은 기존처럼 즉시 등록. 마이그레이션 005, 신규 모듈 `src/sales/consignment.py`
- [x] pytest 118개 전체 통과 (신규 13건: 위탁판매 백엔드 10 + 마이그레이션 1 + 골든패스 1 + 단가 검증 1)
- [x] 브라우저로 위탁판매 전체 흐름 검증 (출고 → 재고 차감 확인 → 부분 판매 확정 → 정산 리포트 반영 확인)
