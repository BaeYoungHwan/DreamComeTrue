...
## 📌 다음 세션 메모 (2026-07-21 세션 종료 시점)

- P1.4(QA 피드백 5건)는 **pytest로만 검증됨** — 수정 후 실제 브라우저 화면으로 재확인은 아직 안 함. 다음 세션에서 QA 데모 앱을 다시 띄워 5건이 실제로 잘 보이는지 재확인 필요.
- QA 데모 DB(`qa_demo.db`)와 검증 스크립트(`seed_qa_demo.py`, `verify_excel_export.py`)는 이번 세션 전용 임시 폴더(scratchpad)에 있어서 **세션이 끝나면 사라질 수 있음** — 다음 세션에서 재확인하려면 데모 데이터를 다시 시딩해야 할 가능성 높음.
- 이전에 띄워둔 Streamlit 서버(포트 8501)는 세션 종료 시점에 중지됨 — 다음에 이어서 QA하려면 재기동 필요.
- 재QA 체크리스트: ① 재고 단위 공백 정상 표시 ② 정산 리포트 "상세 리포트 보기" 정상 동작 ③ 가격 입력란에 소수점 안 뜨는지 ④ 품목 상세에 커스텀 속성 UI 사라졌는지 ⑤ "재고 현황" 탭이 첫 번째 탭으로 잘 보이는지
