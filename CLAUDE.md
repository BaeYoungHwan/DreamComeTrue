...
### 프로젝트 개요
- 프로젝트명: local-farm-inventory (로컬 팜 인벤토리)
- 설명: 1000평 규모 다품종 소량 생산 농가를 위한 로컬 출하용 재고 관리 및 매출 정산 시스템

### Claude 행동 지침
- DB 스키마(데이터 구조) 변경 시 반드시 마이그레이션 코드를 함께 제공할 것
...
### 기술 스택 고정
- Python 3.12 + Streamlit (최신 안정 버전) + SQLite (Python 내장)
- 다른 라이브러리·프레임워크 임의 도입 금지
