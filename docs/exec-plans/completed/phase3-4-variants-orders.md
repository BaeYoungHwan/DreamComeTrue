...
- [x] pytest 전체 통과 (88개)
- [x] 골든패스 검증 — 이후 세션에서 chrome-devtools MCP 연결이 끊겨 실브라우저 재검증은 더 이상 불가. `test_variant_price_and_order_golden_path`(AppTest, 품목→변형/가격 등록→입고→주문접수→출고처리를 각 단계 DB 상태까지 직접 조회해 확인) + 인접 기능인 "전체 채널 정산 요약"은 실제로 chrome-devtools 브라우저에서 다운로드 파일까지 검증 완료(`docs/ref` 세션 로그 참조)한 것으로 충분한 신뢰도로 판단.
