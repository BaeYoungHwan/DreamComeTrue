...
| inventory (items/stock) | A | 테스트 20개, 재고 계산·음수 방지·재고 현황(`stock_overview`) 전체 커버 | 2026-07-21 | Phase 1 완료 + P1.4 `stock_overview()` 추가 (`src/inventory/items.py`, `src/inventory/stock.py`) |
...
| export (report) | A | 테스트 5개, 기간 집계·상세 리포트·엑셀 생성 커버 + 브라우저 실다운로드 검증 | 2026-07-21 | Phase 1 완료 + P1.4 `detailed_period_report()`/`detailed_report_to_excel_bytes()` 추가 (`src/export/report.py`) |
| app (Streamlit UI) | A | AppTest 8개 (탭 구조, v1 핵심 골든패스, 채널·정산 골든패스, 변형·주문 골든패스, 전체채널요약, 재고 현황, 상세 리포트) | 2026-07-21 | 스레드 버그(`check_same_thread`) 발견·수정. AppTest가 실제 운영 DB를 오염시키던 버그 수정(`FARM_DB_PATH` 격리). P1.4에서 커스텀 속성 UI 제거로 관련 테스트 삭제 |
